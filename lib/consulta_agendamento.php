...
if($tipo == 1){
	...
	$sql = "SELECT nome, tipo, status, a.horario, a.data, fil.cidade, fil.bairro, fil.estado, fil.rua, fil.numero FROM agendamento a INNER JOIN agendamentoavalfisicamensal f ON (a.horario = f.horario and a.data = f.data) INNER JOIN pessoa p ON (f.cpf = p.cpf) INNER JOIN filial fil ON (a.idFilial = fil.IdFilial) WHERE a.status != 'Cancelado' AND a.idFilial = '$filial'";
	$result = $conn->query($sql);
	mysqli_close($conn);
	include ('../conectar.php');
	$sql2 = "SELECT nome, tipo, status, a.horario, a.data, fil.cidade, fil.bairro, fil.estado, fil.rua, fil.numero FROM agendamento a INNER JOIN agendamentoaulaexp e ON (a.horario = e.horario and a.data = e.data) INNER JOIN filial fil ON (a.idFilial = fil.IdFilial) WHERE a.status != 'Cancelado' AND a.idFilial = '$filial'";
	$result2 = $conn->query($sql2);
}
